...
const Initialize: NextPageWithLayout = () => {
  const { wallet, publicKey, requestBulkTransactions } = useWallet();
  ...
  const handleSubmit = async (event: ChangeEvent<HTMLFormElement>) => {
    ...
    console.log({ symbol, url, total })
    if (!publicKey) throw new WalletNotConnectedError();

    try {
      const networkClient = await fetch('https://vm.aleo.org/api/testnet3/blocks?start=165380&end=165390');
      const blocksOnChain = await networkClient.json();
      //let block = await networkClient.getBlockRange(165380, 165390);
      const finalNFTs = [];
      for (let i = 0; i < blocksOnChain.length; i++) {
        let blocks = [];
        for (let transaction of blocksOnChain[i]['transactions']) {
          if (transaction?.rejected?.execution?.transitions)
            for (let transition of transaction?.rejected?.execution?.transitions) {
              console.log(transition.program);
              if (transition.program == 'duck_nft_001.aleo') {
                finalNFTs.push(transition);
              }
            }
        }
      }
      console.log("final nfts: " + JSON.stringify(finalNFTs));
    } catch (error) {
      console.log(error);
    }
    ...
    // console.log({ symbolInput, urlInputs, formattedUrlInput, inputs })
